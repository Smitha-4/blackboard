window.onload = function () {
    ...
    // Function to create an off-screen canvas with the entire drawing
    function createFullSizeImage() {
        if (strokes.length === 0) {
            // Return an empty canvas if there are no strokes
        ...
        const buffer = 50;
        const width = maxX - minX + 2 * buffer;
        const height = maxY - minY + 2 * buffer;
        const tempCanvas = document.createElement('canvas');
        const tempCtx = tempCanvas.getContext('2d');
        tempCanvas.width = width;
        tempCanvas.height = height;
        tempCtx.fillStyle = '#000000ff';
        tempCtx.fillRect(0, 0, width, height);
        tempCtx.lineCap = 'round';
        tempCtx.lineJoin = 'round';

        // Draw all strokes onto the temporary canvas without offset
        strokes.forEach(stroke => {
            ...
            if (stroke.type === 'draw' || stroke.type === 'erase') {
                ...
                tempCtx.beginPath();
                const startPoint = stroke.points[0];
                tempCtx.moveTo(startPoint.x - minX + buffer, startPoint.y - minY + buffer);

                for (let i = 1; i < stroke.points.length; i++) {
                    const point = stroke.points[i];
                    tempCtx.lineTo(point.x - minX + buffer, point.y - minY + buffer);
                }
                tempCtx.stroke();
            } else if (stroke.type === 'line') {
                const start = stroke.points[0];
                const end = stroke.points[1];
                tempCtx.beginPath();
                tempCtx.moveTo(start.x - minX + buffer, start.y - minY + buffer);
                tempCtx.lineTo(end.x - minX + buffer, end.y - minY + buffer);
                tempCtx.stroke();
            } else if (stroke.type === 'rect') {
                const start = stroke.points[0];
                const end = stroke.points[1];
                tempCtx.beginPath();
                tempCtx.strokeRect(start.x - minX + buffer, start.y - minY + buffer, end.x - start.x, end.y - start.y);
            } else if (stroke.type === 'circle') {
                const start = stroke.points[0];
                const end = stroke.points[1];
                const radius = Math.sqrt(Math.pow(end.x - start.x, 2) + Math.pow(end.y - start.y, 2));
                tempCtx.beginPath();
                tempCtx.arc(start.x - minX + buffer, start.y - minY + buffer, radius, 0, 2 * Math.PI);
                tempCtx.stroke();
            }
        });
    ...
    savePngButton.addEventListener('click', () => {
        const fullSizeCanvas = createFullSizeImage();
        const image = fullSizeCanvas.toDataURL('image/png');
    ...
    savePdfButton.addEventListener('click', () => {
        ...
        const fullSizeCanvas = createFullSizeImage();
        const imgData = fullSizeCanvas.toDataURL('image/png');
        const pdf = new jsPDF({
            orientation: fullSizeCanvas.width > fullSizeCanvas.height ? 'landscape' : 'portrait',
            unit: 'px',
            format: [fullSizeCanvas.width, fullSizeCanvas.height]
        });
        pdf.addImage(imgData, 'PNG', 0, 0, fullSizeCanvas.width, fullSizeCanvas.height);
